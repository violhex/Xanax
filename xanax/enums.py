"""
Enumerations for Wallhaven API parameters.

These enums provide type-safe options for search parameters and ensure
...
class Sort(StrEnum):
    ...
    - DATE_ADDED: Sort by upload date
...
class Color(StrEnum):
    """
    Valid colors for color-based searching.

    These correspond to the color options available in Wallhaven's
    color filter.
    """
...
class FileType(StrEnum):
    ...
    Note: jpg maps to jpeg in the API.
    """
...
class Ratio:
    """
    Helper class for validating aspect ratios.

    Ratio format: WIDTH:HEIGHT (e.g., "16:9")
    """
...
class Seed:
    """
    Helper class for generating and validating random seed.

    Seed format: 6 alphanumeric characters
    """
